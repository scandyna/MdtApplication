...
Install the dependencies:
```bash
conan install --output-folder . --profile:build linux_gcc13_x86_64 --profile:host linux_ubuntu-24.04_gcc13_x86_64_qt_and_more --settings:build build_type=Release --settings:host build_type=Debug --options:host "&:build_only_doc=True" ..
```
